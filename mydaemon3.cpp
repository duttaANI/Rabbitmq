...
void senderThread(amqp_connection_state_t conn) {
    ...
    amqp_connection_state_t conn_s = amqp_new_connection();
    amqp_socket_t *socket = amqp_tcp_socket_new(conn_s);
    amqp_socket_open(socket, "localhost", 5672);
#endif

    // Create channel
    amqp_channel_open(conn_s, 3);
    amqp_rpc_reply_t channel_reply = amqp_get_rpc_reply(conn_s);
    if (channel_reply.reply_type != AMQP_RESPONSE_NORMAL) {
        std::cerr << "Error opening channel senderThread" << std::endl;
        return;
    }

    // Declare exchange
    amqp_exchange_declare(conn_s, 3, amqp_cstring_bytes(EXCHANGE_NAME.c_str()),
                          amqp_cstring_bytes("direct"), 0, 0, 0, 0, amqp_empty_table);
    amqp_rpc_reply_t exchange_reply = amqp_get_rpc_reply(conn_s);
    ...
    while (counter--) {
        ...
        amqp_basic_publish(conn_s, 3, amqp_cstring_bytes(EXCHANGE_NAME.c_str()),
    ...
    amqp_channel_close(conn_s, 3, AMQP_REPLY_SUCCESS);
    amqp_connection_close(conn_s, AMQP_REPLY_SUCCESS);
    amqp_destroy_connection(conn_s);
#endif
...
void receiverThread(amqp_connection_state_t conn) {
...
#if 1
	// Connect to RabbitMQ server
    amqp_connection_state_t conn_r = amqp_new_connection();
    amqp_socket_t *socket = amqp_tcp_socket_new(conn_r);
    amqp_socket_open(socket, "localhost", 5672);
#endif

    // Create channel
    amqp_channel_open(conn_r, 3);
    amqp_rpc_reply_t channel_reply = amqp_get_rpc_reply(conn_r);
    if (channel_reply.reply_type != AMQP_RESPONSE_NORMAL) {
        std::cerr << "Error opening channel receiverThread" << std::endl;
        return;
    }
    ...
    // Declare queue
    amqp_queue_declare(conn_r, 3, amqp_cstring_bytes(QUEUE_NAME.c_str()), 0, 0, 0, 1,
                       amqp_empty_table);
    amqp_rpc_reply_t queue_reply = amqp_get_rpc_reply(conn_r);
    ...
    amqp_queue_bind(conn_r, 3, amqp_cstring_bytes(QUEUE_NAME.c_str()),
                    amqp_cstring_bytes(EXCHANGE_NAME.c_str()),
                    amqp_cstring_bytes(ROUTING_KEY.c_str()), amqp_empty_table);
    amqp_rpc_reply_t bind_reply = amqp_get_rpc_reply(conn_r);
    if (bind_reply.reply_type != AMQP_RESPONSE_NORMAL) {
        std::cerr << "Error binding queue to exchange receiverThread" << std::endl;
	if (bind_reply.reply_type == AMQP_RESPONSE_LIBRARY_EXCEPTION) {
                std::cerr << "Library Exception consuming message Error binding queue to exchange receiverThread: " << bind_reply.library_error << std::endl;
            }
        return;
    }
    ...
    while (counter--) {
        ...
        amqp_maybe_release_buffers(conn_r);
	std::cout << "Before amqp_consume_message" << std::endl;
        amqp_rpc_reply_t consume_reply = amqp_consume_message(conn_r, &envelope, nullptr, 1000);
	std::cout << "After amqp_consume_message" << std::endl;
	    ...
	    if (consume_reply.reply_type == AMQP_CHANNEL_CLOSE_METHOD) {
		    amqp_channel_close(conn_r, 3, AMQP_REPLY_SUCCESS);
		    std::cout << "Channel closed by server. Attempting to reopen." << std::endl;
		    reconnect(conn_r);
		    continue;
	    }
	    else if (consume_reply.reply_type == AMQP_RESPONSE_LIBRARY_EXCEPTION) {
        	std::cerr << "Library Exception consuming message receiverThread: " << consume_reply.library_error << std::endl;
	    }
            continue;
        }
    ...
    // Close the connection
    amqp_channel_close(conn_r, 3, AMQP_REPLY_SUCCESS);
    amqp_connection_close(conn_r, AMQP_REPLY_SUCCESS);
    amqp_destroy_connection(conn_r);
#endif
