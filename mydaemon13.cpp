...
void senderThread(amqp_connection_state_t conn) {
    ...
    while (counter < 5) {
        ...
        amqp_basic_publish(conn, 1, amqp_empty_bytes, amqp_cstring_bytes(QUEUE_NAME.c_str()), 0, 0, &props,
...
void receiverThread(amqp_connection_state_t conn) {
    ...
    // Bind the queue
    amqp_queue_bind(conn, 2, amqp_cstring_bytes(QUEUE_NAME.c_str()), amqp_empty_bytes, amqp_empty_table);
